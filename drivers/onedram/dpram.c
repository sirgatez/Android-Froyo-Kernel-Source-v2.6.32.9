...
#if defined(CONFIG_ONEDRAM_HWREV)
...
static int 
dpram_write(dpram_device_t *device,	const unsigned char *buf, int len)
{
	int retval = 0;
	int size = 0;
...
static int dpram_shared_bank_remap(void)
{
	...
	/*
	 * Set CP onedram interrupt pin to low befor CP HW logic init by phone pmic.
	 * If it was kept high, some device fail the phone booting.
	 * nINT_ONEDRAM_CP, RESET_REQ_N, GPIO_PDA_ACTIVE
	 */
	*onedram_mailboxBA=0x0;
	return 0;
}
...
static int dpram_phone_boot_start(void)
{
	...
	*onedram_mailboxBA = send_mail;
	...
	else
	{
	    ...
	    while(gpio_get_value(GPIO_INT_ONEDRAM_AP_N))
			printk(".");
		printk("\n");
...
static void init_hw_setting(void)
{
	...
	if (gpio_is_valid(GPIO_PDA_ACTIVE)) {
		...
		/*
		 * PDA_ACTIVE will set high after phone on for fixing CP boot fail.
		 */
